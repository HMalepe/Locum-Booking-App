import db from './db';
import { hoursBetween, cutoffAfter, fmtDate } from './format';
...
export function ensurePaymentNudges(manager) {
  ...
  // The weekly nudge also lands in the manager's inbox (Resend)
  if (nudgedNow > 0) {
    const isOverdue = overdue.length > 0;
    sendEmail({
      to: manager.email,
      subject: isOverdue
        ? `🚨 Locum payments OVERDUE — ${unpaid.length} unpaid shift${unpaid.length === 1 ? '' : 's'}`
        : `🔔 Weekly reminder: ${unpaid.length} unpaid locum shift${unpaid.length === 1 ? '' : 's'}`,
      heading: isOverdue ? 'Locum payments overdue!' : 'Weekly payment reminder',
      lines: [
        `You have ${unpaid.length} unpaid shift${unpaid.length === 1 ? '' : 's'}${totalDue > 0 ? ` totalling ± R ${totalDue.toLocaleString()}` : ''}.`,
        isOverdue
          ? `${overdue.length} ${overdue.length === 1 ? 'is' : 'are'} past your payment cutoff — please pay your locums now.`
          : nextPayBy
            ? `Your cutoff for these is ${fmtDate(nextPayBy)}. This reminder repeats weekly until everything is marked paid.`
            : 'No payment cutoff is set — add one in Settings so locums know when to expect payment.',
      ],
      ctaLabel: 'Review & mark paid',
      ctaPath: '/payments',
    });
  }
